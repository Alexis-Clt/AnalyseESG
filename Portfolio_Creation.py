# -*- coding: utf-8 -*-
"""
Created on Wed Jun  1 14:27:40 2022
...
def portfolioCalc(weight,data,name):
    data[name] = sum([int(weight[x])*data[x] /100 for x in list(weight.keys()) ])
    return data 
    # %% Normalisation
def Normalisation(basedata,members):
    for stock in members:
        basedata[stock] = basedata[stock]/basedata[stock].iloc[0]
    return basedata
    #print(basedata) 
...
test=Algo(0.7,0.2,0.1,85,start_date,end_date)
DisplayResults(test)
ToString(test)
...
start_date_observation="2021-03-01"
end_date_observation="2021-05-01"
