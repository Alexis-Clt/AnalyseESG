...
class Portefeuille :
    ...
    def vendre(self, action, nombre=1):
        ...
        while(vendues<nombre and self.titres.at[ligne[0], "Nombre"]>0):
             vendues +=1
             self.soldeEspece += prixActuel*(1-self.frais_vente)
             self.titres.at[ligne[0], "Nombre"]-=1
        return vendues
...
if __name__ == "__main__":
    ...
    frais_achat = 0
    frais_achat = 0
    notrePortefeuille = Portefeuille(solde_initial, path, scoreESG)
    notrePortefeuille.acheter("AXA")
    notrePortefeuille.acheter("AXA", 2)
    notrePortefeuille.verserDividende("AXA")
    notrePortefeuille.verserDividende("BNP Paribas")
